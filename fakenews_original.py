"""
Simulação sequencial de Propagação de Fake News (versão original do professor)
...
def simular_sem_imprimir(
    ...
    grade = criar_grade(linhas, colunas, percentual_espalhadores, semente)

    for _ in range(geracoes):
        grade = proxima_geracao(grade, limiar_convencimento)
        if contar_estados(grade)[ESPALHADOR] == 0:
            break
